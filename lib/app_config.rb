class AppConfig
  ...
  class << self
    ...
    def load_features_config(config)
      ...
      config.max_points_per_tip = parse_positive_int("MAX_POINTS_PER_TIP", 100)
      ...
      config.default_inline_quantity = parse_positive_int("DEFAULT_INLINE_QUANTITY", 1)
      config.default_reaction_quantity = parse_positive_int("DEFAULT_REACTION_QUANTITY", 1)
