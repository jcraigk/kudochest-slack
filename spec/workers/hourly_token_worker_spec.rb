...
RSpec.describe HourlyTokenWorker do
  ...
  let(:current_hour) { Time.current.utc.hour }
  let(:frequency) { 'weekly' }
  let(:mock_relation) { instance_spy(ActiveRecord::Relation) }
  let(:current_time) { Time.zone.local(2019, 11, 8, 21, 1, 1).utc }
  ...
  before do
    ...
    allow(TokenDisbursalWorker).to receive(:perform_async)
  ...
  it 'calls TokenDisbursalWorker on monthly team once' do
    expect(TokenDisbursalWorker).to have_received(:perform_async).with(team4.id)
  end
