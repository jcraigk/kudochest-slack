...
RSpec.describe Cache::TeamConfig do
  ...
  let(:regex) { { regex: "(?<match>(?:<(?<entity_rid>(?:@|\\#|!subteam\\^)[A-Z0-9]+)(?:\\|[^>]*)?>|)\\s{0,20}(?<prefix_quantity>\\d+\\.?\\d*)?\\s?(?:(?<inlines>(?:\\+\\+)+|(?:\\+=)+|(?:\\+)+|(?:\\-\\-)+|(?:\\-=)+|(?:\\-)+)|(?<emojis>(?:(?::#{App.default_point_emoji}:|:#{App.default_jab_emoji}:)\\s*)+))\\s?(?<suffix_quantity>\\d+\\.?\\d*)?\\s{0,20}(?<topic_keywords>#{(team.topics.map(&:keyword) + topic_emojis).join('|')})?)" } } # rubocop:disable Layout/LineLength
